...
void URadarComponent::TickComponent(float DeltaTime, ELevelTick Tick, FActorComponentTickFunction* MovieSceneBlends)
{
	Super::TickComponent(DeltaTime, Tick, MovieSceneBlends);
...
void URadarComponent::SetVisibility(const bool IsVisible, const bool CanAnimate)
{
...
void URadarComponent::ToggleVisibility(const bool CanAnimate)
{
	SetVisibility(!bVisible, CanAnimate);
}
...
{
	if (!Enemies.Contains(Enemy) && RadarWidget)
	{
		Enemies.Add(Enemy);
...
void URadarComponent::OnEnterSecretRange(TScriptInterface<IInteractable> Interactable)
{
	if (RadarWidget && Interactable.GetObject())
	{
...
void URadarComponent::OnLeaveSecretRange(TScriptInterface<IInteractable> Interactable)
{
	if (RadarWidget && Interactable.GetObject())
	{
		if (Interactables.Contains(Interactable))
...
void URadarComponent::UpdatePositionForEnemy(ACharacter* Enemy) const
{
	...
	if (EnemyIndex >= 0)
	{
		...
		if (RadarWidget && RadarWidget->GetEnemyWidgets().Num() > 0)
		{
