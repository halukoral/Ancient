...
void UPlaceableManager::BeginPlay()
{
	...
	if(Player)
	{
		...
		if(PlayerCamera == nullptr || FortniteComponent == nullptr || ResourceManager == nullptr)
		{
			return;
		}
...
{
	Placeable ? ChangePreviewMesh() : SpawnPreviewMesh();  
}
...
{
	if (FortniteComponent && Placeable && !Placeable->IsOverlapping() && IsResourceEnough())
	{
		...
		FortniteComponent->SetPlaceableOnSight(Placeable);
