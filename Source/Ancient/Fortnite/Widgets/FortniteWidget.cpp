	...
	if(ResourceManager)
	{
		...
		for (uint8 i = 0; i < ResourceData.Num(); ++i)
		{
			...
			if (W_Resource != nullptr)
			{
				...
				ResourceManager->AddWidgets(ResourceData[i]->Type, W_Resource);
				ResourceWidgets.Add(ResourceData[i]->Type, W_Resource);
				
				const auto ResourceSlot { ResourceBox->AddChildToHorizontalBox(W_Resource) };
				if (i > 0)
				{
	...
	if(PlaceableManager)
	{
		...
		for (uint8 i = 0; i < PlaceableWidgetData.Num(); ++i)
		{
			...
			if (W_Placeable != nullptr)
			{
				...
				PlaceableManager->AddWidgets(PlaceableWidgetData[i]->Type, W_Placeable);

				const auto BuildingSlot = BuildingBox->AddChildToHorizontalBox(W_Placeable);
				if (i > 0)
				{
